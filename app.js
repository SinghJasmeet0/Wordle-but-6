...
for(let i=0; i<6;i++){
    ...
    line.setAttribute('id', 'line'+(i))
        for (let id=0; id<6;id++){
            ...
            square.setAttribute('id' ,'line-' + i + '-item-'+id )
            square.classList.add('box');
            line.append(square);
        }
    gameContainer.append(line);
      
}


 function insertLetter(letter){
        let selectedBox = document.getElementById('line-'+currentLine + '-item-'+currentBox);
        
        if(gameOver === false){
            if(currentLine<6 && currentBox<=6){
                if(letter === 'ENTER'){
                    console.log('ent');
                    
                    if(currentBox === 6){
                       fetch(url+currentGuess.join(''))
                       .then(res =>{
                        if(res.ok){
                            checkForAns();
                            currentLine++;
                            ...
                            let wrong = document.getElementById('line'+currentLine)
                            wrong.classList.add('red');
                            setTimeout(() => {
                                wrong.classList.remove('red');
                            }, 150);
                        }
                       })
                    ...
                    if(currentBox>=6){
                        ...
                        deleteLetter(selectedBox);
                    }
                }else if(currentBox<6) { 
                        selectedBox.innerHTML = letter;
                        currentGuess[currentBox] = letter;
                        currentBox++;
...
function deleteLetter(box){
    ...
    currentGuess[currentBox]= '';
    box.textContent = '';  
}
   

function checkForAns(){
    const rowBox = document.querySelector('#line'+ currentLine).childNodes
    rowBox.forEach((box,i)=>{
        setTimeout(()=> {
            box.classList.add('flip')
            const data = box.innerHTML;
            if(data == checkWordle[i]){
    ...
    rowBox.forEach((box,i)=>{
        setTimeout(()=> {
            const data = box.innerHTML;
            if(checkWordle.includes(data)){
                box.classList.add('yellow');
                checkWordle[checkWordle.indexOf(data)] = ' ';
            }else{
...
function showMessage(){
    ...
    if(currentWord == wordle){
        msgContainer.innerHTML = ('Magnificent')
        msgContainer.classList.add('inmsg')
        setTimeout(() => {
            msgContainer.classList.remove('inmsg')
        }, 5000);
        gameOver = true;
    }else if(currentLine>=6){
        msgContainer.innerHTML = (wordle);
        msgContainer.classList.add('inmsg')
        setTimeout(() => {
            msgContainer.classList.remove('inmsg')
        }, 5000);
